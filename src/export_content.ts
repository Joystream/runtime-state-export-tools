...
import {
  ...
  Class,
  Entity,
} from '@joystream/types/versioned-store'
...
type ExportedEntities = Array<{
  entity: Entity
  maintainer: Credential | undefined
}>
...
async function main() {
  ...
  const versioned_store_data = {
    ...
    entities: exportedEntities.map(({ entity, maintainer }) => ({
      entity: entity.toHex(),
...
async function get_checked_map_value<T extends Codec>(
  api: ApiPromise,
  module: string,
  map: string,
  key: CodecArg
...
async function get_all_entities(api: ApiPromise): Promise<ExportedEntities> {
  ...
  for (let id = first; id < next; id++) {
    ...
    const maintainerStorageKey = api.query.versionedStorePermissions.entityMaintainerByEntityId.key(
      id
    )
    const maintainerEntry = ((await api.rpc.state.getStorage(
      maintainerStorageKey
    )) as unknown) as Option<Tuple>
    const maintainer = maintainerEntry.isSome
      ? new SingleLinkedMapEntry<EntityId, Credential>(
          EntityId,
          Credential,
...
async function get_channels(api: ApiPromise) {
  ...
  for (let id = firstChannelId; nextChannelId.gtn(id); id++) {
    const channel = (await api.query.contentWorkingGroup.channelById(
      id
    )) as Channel
