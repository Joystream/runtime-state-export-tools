...
import { Profile, MemberId, ActorInRole, Role, ActorId, RoleKeys } from '@joystream/types/members'
...
import { Curator, CuratorId, Lead, LeadId } from '@joystream/types/content-working-group'
...
async function main () {
    ...
    const storageWorkerAccounts = await enumerate_storage_working_group_worker_accounts_and_stakes(api)
    const contentWgWorkerAccounts = await enumerate_content_wg_accounts_and_stakes(api)
...
async function enumerate_proposal_stakes(api: ApiPromise) : Promise<Array<MemberAndStake>> {
...
async function enumerate_storage_working_group_worker_accounts_and_stakes (api: ApiPromise) {
...
async function enumerate_content_wg_accounts_and_stakes(api: ApiPromise) {
    ...
    // lead role account
    const maybeLeadId = (await api.query.contentWorkingGroup.currentLeadId()) as unknown as Option<LeadId>
...
async function memberIdFromRoleAndActorId (api: ApiPromise, role: Role, id: ActorId): Promise<MemberId> {
    const memberId = (
      await api.query.members.membershipIdByActorInRole(
        new ActorInRole({
          role: role,
          actor_id: id
        })
...
function memberIdFromCuratorId (api: ApiPromise, curatorId: CuratorId): Promise<MemberId> {
    return memberIdFromRoleAndActorId(
        api,
...
async function enumerate_rewarded_accounts(api: ApiPromise) {
    ...
    rewards.forEach(reward => accounts.add(reward.getField('account').toString()))
